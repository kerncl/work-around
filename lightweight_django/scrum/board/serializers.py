    ...
    def get_links(self, obj):
        ...
        return {
            ...
            'tasks': reverse('task-list',
                             request=request) + f'?sprint={obj.pk}'
        }
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    links = serializers.SerializerMethodField() # Show next url in the api page
        ...
        if self.instance and self.instance.pk:
            if value != self.instance.sprint:
                if self.instance.status == Task.STATUS_DONE:
                    msg = ugettext_lazy('Cannot change the spint of a completed task')
                    raise serializers.ValidationError(msg)
                if value and value.end < date.today():
                    msg = ugettext_lazy('Cannot assign tasks to past sprints')
                    raise serializers.ValidationError(msg)
        else:
            if value and value.end < date.today():
                msg = ugettext_lazy('Cannot add tasks to past sprints')
                raise serializers.ValidationError(msg)
        return value
    ...
    def validate(self, attrs):
        ...
        if not sprint and status != Task.STATUS_TODO:
            msg = ugettext_lazy("Backlog tasks must have 'Not Started' status.")
            raise serializers.ValidationError(msg)
        if started and status == Task.STATUS_TODO:
            msg = ugettext_lazy('Started date cannot be set for started tasks')
            raise serializers.ValidationError(msg)
        if completed and status != Task.STATUS_DONE:
            msg = ugettext_lazy('Completed date cannot be set for uncompleted tasks')
            raise serializers.ValidationError(msg)
        return attrs
